...
def get_number_posts_profile_page(driver):
    number_posts = fnum.string_to_number(driver.find_elements_by_xpath(".//span[@class = '_fd86t' ]")[0].text)
    time.medium_timeout()
    return number_posts


def get_number_followers_profile_page(driver):
    number_followers =  fnum.string_to_number(driver.find_elements_by_xpath(".//span[@class = '_fd86t' ]")[1].text)
    time.medium_timeout()
    return number_followers

def get_number_following_profile_page(driver):
    number_following = fnum.string_to_number(driver.find_elements_by_xpath(".//span[@class = '_fd86t' ]")[2].text)
    time.medium_timeout()
    return number_following

def get_username_profile_page(driver):
    username = driver.find_element_by_xpath(".//h1[@class = '_rf3jb notranslate']").text
    time.medium_timeout()
    return username
...
def click_unfollow_popup_list(element):
    element.find_element_by_xpath(".//button[@class = '_qv64e _t78yp _4tgw8 _njrw0']").click()
    time.medium_timeout()