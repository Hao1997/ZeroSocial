...
def add_unregisted_followers_to_database(number_users):
    ...
    insta_actions.click_followers_profile_page(driver)
    for i in range(0, number_users):
        ...
        while (len(popupbox) - 1 < i):
            try:
                ...
                popupbox = insta_actions.get_all_users_popup_list()
                print('scrolling')
            except Exception as e:
                continue;
        driver.execute_script("arguments[0].scrollIntoView(true);", popupbox[len(popupbox) - 1])
...
def add_unregisted_follower_to_database():
    ...
    if(not database.find_instagram_user_by_name(username)):
        database.add_instagram_user(username=username, posts = post, followers= followers, following=following, isFollowing=True);
add_unregisted_followers_to_database(200)
driver.close()